...
async function uploadPdf(_container: unknown, file = createPdfFile()) {
  ...
  const input = await waitFor(() => {
    const el = document.getElementById('pdf-upload-input') ?? document.querySelector('input[type="file"]')
...
describe('App', () => {
  ...
  it('keeps upload view when clicking análisis without transactions', async () => {
    ...
    expect(screen.getByRole('heading', { name: /entiende tus gastos sin complicarte/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /inicio/i })).toHaveClass('active')
  })
  ...
  it('shows a recoverable error state and allows re-upload after empty extraction', async () => {
    ...
    expect(await screen.findByText(/no se encontraron transacciones en el pdf/i)).toBeInTheDocument()
