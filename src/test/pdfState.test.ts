...
import { savePdfState, loadPdfState, clearPdfState, hasPdfState } from '../services/pdfState';
...
describe('pdfState service', () => {
  ...
  describe('savePdfState', () => {
    it('saves valid PDF state to localStorage', () => {
      ...
      const result = savePdfState(state as unknown as Partial<PdfState>);

      expect(result.success).toBe(true);
      expect(mockLocalStorage.setItem).toHaveBeenCalledWith('pdfState', expect.any(String));
    });
    ...
    it('handles quota exceeded error gracefully', () => {
      ...
      mockLocalStorage.setItem = vi.fn((key, _value) => {
        if (key === 'pdfState') {
