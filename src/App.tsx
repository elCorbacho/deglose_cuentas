...
export default function App() {
    ...
    <>
      <section className="hero-shell view-panel">
        <div className="relative z-10 flex flex-col gap-2">
          ...
          <p className="max-w-lg text-sm leading-6" style={{ color: 'var(--text-soft)' }}>
            Subí tu estado de cuenta Santander y explorá tus movimientos por categoría, con filtros
            de fecha y totales al instante.
          </p>
        </div>
      ...
      {loading && (
        <section className="panel status-card status-card--loading view-panel">
          <div className="spinner-ring inline-block h-10 w-10 animate-spin rounded-full" />
      ...
      {error && !loading && (
        <section className="panel status-card status-card--error view-panel">
          <div className="error-icon flex h-10 w-10 items-center justify-center rounded-full">
            <svg
          ...
          <div className="space-y-1">
            ...
            <p className="max-w-md text-sm" style={{ color: 'var(--text-soft)' }}>
              {error}
            </p>
          </div>
  ...
  const renderAnalysisView = () => {
    ...
    let progress = 0;
    const _donutGradient = chartItems.length
          ...
          <>
            <div className="analysis-row analysis-widgets-grid analysis-widgets-grid--two">
              <div className="panel widget-card total-card">
                ...
                {/* Main amount */}
                <p
                  className="widget-total-amount mono-num total-card__amount"
                  style={{ color: 'var(--text-strong)' }}
                >
  ...
  const renderConfigView = () => (
    <section className="panel view-panel p-5">
      ...
      <div className="min-h-screen">
        ...
        <div className="app-layout">
          ...
          <main className="content-area">
            <div className="app-shell app-shell--compact">{renderCurrentView()}</div>
          </main>
        </div>
      </div>
    </TooltipProvider>
